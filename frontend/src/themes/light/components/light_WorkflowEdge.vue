...
const edgeMarkerEnd = computed(() => {
  const base = (typeof props.markerEnd === 'object' && props.markerEnd !== null)
    ? { type: MarkerType.Arrow, width: 18, height: 18, color: '#4b5563', strokeWidth: 2, ...props.markerEnd }
    : props.markerEnd
...
const labelStyle = computed(() => {
  // Base style - light theme for contrast on light canvas
  const s = {
    position: 'absolute',
...
@keyframes label-pulse {
  0%, 100% {
    box-shadow: 0 0 1px inset rgba(0, 0, 0, 0.05);
    background-color: rgba(255, 255, 255, 0.98);
  }
  50% {
    box-shadow: 0 0 8px inset rgba(75, 85, 99, 0.15);
    background-color: rgba(255, 255, 255, 0.98);
  }
}
